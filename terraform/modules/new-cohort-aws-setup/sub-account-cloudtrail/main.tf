# TODO: Need to add Cloudtrail to the subaccount
terraform {
  required_providers {
    aws = {
...
locals {
  ...
  trail_name  = "CodeYou-Trail-${local.account_id}-${local.region}"
  subaccount_trail_bucket_name = "${var.subaccount_cloudtrail_bucket_name}-${var.environment}"
...
resource "aws_kms_alias" "cloudtrail_key_alias" {
  name          = "alias/cloudtrail-key-${local.region}-${local.environment}"
  target_key_id = aws_kms_key.cloudtrail_key.id
}

# Create an S3 bucket for CloudTrail logs
resource "aws_s3_bucket" "cloudtrail_bucket" {
  bucket = local.subaccount_trail_bucket_name
...
resource "aws_cloudtrail" "CodeYou_student_trail" {
  ...
  tags = {
    Name        = "Code:You - Student Account CloudTrail"
    Environment = local.environment
  }
}
